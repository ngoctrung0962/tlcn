    ...
    return (
        <>
            ...
            <Controller
                ...
                render={({field}) => (
                    <Form.Group className="mb-3">
                        <Form.Label>Thời gian bắt đầu sự kiện </Form.Label>
    ...
    const handleNext = async (data) => {
        ...
        if (activeStep === steps.length - 1) {
            try {
                ...
                console.log(data);
                if(data?.until){
                    data.until = data.until.replaceAll('-', '');
                }
                const res = await calendarApi.addCalendar(data);
                if (res?.errorCode && res.errorCode === "400") {
                    Swal.fire({
                        icon: "fail",
                        title: "Thêm lịch học tập thất bại, vui lòng thử lại",
                    });
                }else{
                    Swal.fire({
                        icon: "success",
                        title: "Thêm lịch học tập thành công vào Google Calendar",
                    });
                }
            } catch (error) {
                console.log(error);
